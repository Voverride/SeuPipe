...
def fit_model(adata, gmt_path, project = None, pre_weights='', label_name='Celltype',max_g=300,max_gs=300, mask_ratio = 0.015,n_unannotated = 1,batch_size=8, embed_dim=48,depth=2,num_heads=4,lr=0.001, epochs= 10, lrf=0.01, train_steps=None, evaluate_result=None):
    GLOBAL_SEED = 1
    ...
    for epoch in range(epochs):
        ...
        if train_steps is not None and evaluate_result is not None:
            train_steps['percent'] = int((epoch+1)/epochs*100)
            evaluate_result['trainAcc'].append(train_acc)
            evaluate_result['valAcc'].append(val_acc)
            evaluate_result['trainLoss'].append(train_loss)
            evaluate_result['valLoss'].append(val_loss)
