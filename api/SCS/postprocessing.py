...
def read_gradient(file, adata, layer_name, patch_size, bin_size, dia_estimate):
    mask2x = {}
    mask2y = {}
    for i in range(adata.layers[layer_name].shape[0]):
        for j in range(adata.layers[layer_name].shape[1]):
            if adata.layers[layer_name][i, j] == 0:
                continue
            if adata.layers[layer_name][i, j] in mask2x:
                mask2x[adata.layers[layer_name][i, j]].append(i)
                mask2y[adata.layers[layer_name][i, j]].append(j)
            else:
                mask2x[adata.layers[layer_name][i, j]] = [i]
                mask2y[adata.layers[layer_name][i, j]] = [j]

    mask2center = {}
    sizes = []
    min_center2center = []
    for nucleus in mask2x:
        mask2center[nucleus] = [np.mean(mask2x[nucleus]), np.mean(mask2y[nucleus])]
        sizes.append(len(mask2x[nucleus]))
    ...
    with open(file) as fr:
        for line in fr:
            ...
            for center in mask2center:
                dis_c = euclidean_distance(mask2center[center][0], mask2center[center][1], int(x), int(y))
                if dis_c <= int(dia_estimate):
                    class_prior += dir_to_class(mask2center[center][0] - int(x) , mask2center[center][1] - int(y), class_num, mask2center[center][0], mask2center[center][1])
                if dis_c <= int(dia_estimate) * 1.25:
                    class_prior_ext += dir_to_class(mask2center[center][0] - int(x) , mask2center[center][1] - int(y), class_num, mask2center[center][0], mask2center[center][1])
...
def result_stats(result_dir, merged, startx, starty, patchsize):
    cell2nspots = {}
    ...
    with open(result_dir+'/cell_stats_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', 'w') as fw:
...
def postprocess(project_path, layer_name, startx, starty, patchsize, bin_size, dia_estimate):
    downrs = bin_size
    startx = str(startx)
    starty = str(starty)
    ...
    cellmask_dir = os.path.join(result_dir, 'cell_mask')
    nuclmask_dir = os.path.join(result_dir, 'nucl_mask')
    figmask_dir = os.path.join(result_dir, 'fig_mask')

    if not os.path.exists(cellmask_dir):
        os.makedirs(cellmask_dir)
    if not os.path.exists(nuclmask_dir):
        os.makedirs(nuclmask_dir)
    if not os.path.exists(figmask_dir):
        os.makedirs(figmask_dir)
    ...
    intensity, dx, dy, pred_U, pred_V, pred_C = read_gradient(result_dir+'/spot_prediction_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', adata, layer_name, (int(patchsizex), int(patchsizey)), bin_size, dia_estimate)
    ...
    fw = open(cellmask_dir+'/spot2cell_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', 'w')
    for i in range(merged.shape[0]):
        for j in range(merged.shape[1]):
            if merged[i, j] > 0:
                fw.write(str(i) + ':' + str(j) + '\t' + str(int(merged[i, j])) + '\n')
    fw.close()

    nucl_labels = adata.layers[layer_name]
    fw = open(nuclmask_dir+'/spot2nucl_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', 'w')
    for i in range(nucl_labels.shape[0]):
        for j in range(nucl_labels.shape[1]):
            if nucl_labels[i, j] > 0:
                fw.write(str(i) + ':' + str(j) + '\t' + str(int(nucl_labels[i, j])) + '\n')
    fw.close()

    result_stats(result_dir, merged, startx, starty, patchsize)
