...
def run_experiment(project_path, startx, starty, patchsize, model, x_train, x_train_pos, x_train_, x_train_pos_, y_train, y_train_, y_binary_train, x_test, x_test_pos, x_validation, x_validation_pos, y_validation, y_binary_validation, learning_rate, weight_decay, batch_size, num_epochs, early_stop_patience=15):
    ...
    checkpoint_filepath = os.path.join(tmp_dir+'/ckpt', 'model_' + startx + '_' + starty + '_' + patchsize, 'ckpt')
    ...
    x_train_pos__ = np.load(tmp_dir+'/x_train_pos_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz')
    x_train_pos__ = x_train_pos__['x_train_pos']
    x_test_pos_ = np.load(tmp_dir+'/x_test_pos_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz')
    x_test_pos_ = x_test_pos_['x_test_pos']
    #print(y_train_.shape, x_train_pos__.shape)

    print('Write prediction results...')
    with open(result_dir+'/spot_prediction_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', 'w') as fw:
        for i in range(len(y_train_)):
...
def train(project_path, startx, starty, patchsize, epochs, val_ratio):
    startx = str(startx)
    starty = str(starty)
    patchsize = str(patchsize)
    tmp_dir = os.path.join(project_path, 'tmp')
    x_train_ = np.load(tmp_dir+'/x_train_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz')
    x_train_ = x_train_['x_train'].astype(np.float32)
    x_train_pos_ = np.load(tmp_dir+'/x_train_pos_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz')
    x_train_pos_ = x_train_pos_['x_train_pos'].astype(np.int32)
    y_train_ = np.load(tmp_dir+'/y_train_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz')
    y_train_ = y_train_['y_train']
    y_binary_train_ = np.load(tmp_dir+'/y_binary_train_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz')
    y_binary_train_ = y_binary_train_['y_binary_train'].astype(np.int32)
    x_test = np.load(tmp_dir+'/x_test_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz')
